...
from django_py3_ldap.ldap import connection, Connection
from django_py3_ldap.utils import clean_ldap_name, import_func


@skipUnless(settings.LDAP_AUTH_ADMIN_DN,
            "Missing settings.LDAP_AUTH_ADMIN_DN")
@skipUnless(settings.LDAP_AUTH_ADMIN_PASSWORD,
            "Missing settings.LDAP_AUTH_ADMIN_PASSWORD")
@skipUnless(settings.LDAP_AUTH_TEST_USER_USERNAME,
            "No settings.LDAP_AUTH_TEST_USER_USERNAME supplied.")
@skipUnless(settings.LDAP_AUTH_TEST_USER_PASSWORD,
            "No settings.LDAP_AUTH_TEST_USER_PASSWORD supplied.")
@skipUnless(settings.LDAP_AUTH_USER_LOOKUP_FIELDS == ("username",),
            "Cannot test using custom lookup fields.")
@skipUnless(django_settings.AUTH_USER_MODEL == "auth.User",
            "Cannot test using a custom user model.")
class TestLdap(TestCase):
    ...
    def testLazySettingsClassLookup(self):
        self.assertEqual(
            settings.__class__.LDAP_AUTH_TEST_USER_USERNAME.name,
            "LDAP_AUTH_TEST_USER_USERNAME"
        )
        self.assertEqual(
            settings.__class__.LDAP_AUTH_TEST_USER_USERNAME.default, "")
    ...
    def testLdapConnection(self):
        with connection() as c:
            self.assertIsInstance(
                c, Connection, "Check LDAP URL and admin credentials")

    def testSearchUserSuccess(self):
        with connection() as c:
            user_ldap_data = c.search_user(
                username=settings.LDAP_AUTH_TEST_USER_USERNAME,
            )
            self.assertTrue(user_ldap_data)

    def testSearchUserIncorrectUsername(self):
        with connection() as c:
            user_ldap_data = c.search_user(
                username="bad" + settings.LDAP_AUTH_TEST_USER_USERNAME,
            )
            self.assertEqual(user_ldap_data, None)
    ...
    def testAuthenticateWithTLS(self):
        with self.settings(LDAP_AUTH_USE_TLS=True):
            ...
            self.assertIsInstance(user, User)
            self.assertEqual(
                user.username, settings.LDAP_AUTH_TEST_USER_USERNAME)
    ...
    def testPromoteMissingUser(self):
        with self.assertRaises(
                CommandError,
                msg="User with username missing_user does not exist"):
    ...
    def testSyncUserRelations(self):
        def check_sync_user_relation(user, data):
            ...
            # model is saved
            self.assertEqual(
                user.username, User.objects.get(pk=user.id).username)
            # save all groups
        ...
        with self.settings(
                LDAP_AUTH_SYNC_USER_RELATIONS=check_sync_user_relation):
            user = authenticate(
                username=settings.LDAP_AUTH_TEST_USER_USERNAME,
                password=settings.LDAP_AUTH_TEST_USER_PASSWORD,
            )
            ...
            self.assertEqual(
                user.groups.filter(name='default_group').count(), 1)

    def testImportFunc(self):
        self.assertIs(clean_ldap_name, import_func(clean_ldap_name))
        self.assertIs(clean_ldap_name,
                      import_func('django_py3_ldap.utils.clean_ldap_name'))
        self.assertTrue(
            callable(import_func('django.contrib.auth.get_user_model')))

        self.assertRaises(AttributeError, import_func, 123)

        self.assertTrue(
            callable(import_func(settings.LDAP_AUTH_SYNC_USER_RELATIONS)))

        with self.settings(
                LDAP_AUTH_SYNC_USER_RELATIONS='django.contrib.auth.get_user_model'):
            self.assertTrue(
                callable(import_func(settings.LDAP_AUTH_SYNC_USER_RELATIONS)))
